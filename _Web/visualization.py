...
df_conf = pd.read_csv("Covid_19.csv")
...
p_DateConfirmed.line(
    x="conf_date", y="conf_num", color="#FC4F4F", alpha=0.5, source=source
)
...
conf2_data = {"months": y, "confirmed": confirmed_list, "color": Category20_17}
...
df_WFH = house()
df_conf1 = pd.read_csv("Covid_19.csv")
...
source1 = ColumnDataSource(data=WFH_data)
source2 = ColumnDataSource(data=WFH_COVID_data)
...
p_Restaurant.vbar(x=Restaurant_month, top=Restaurant_sales, width=0.9)

p_Restaurant.extra_y_ranges["Covid_month"] = Range1d(start=0, end=250)
p_Restaurant.add_layout(
    LinearAxis(y_range_name="Covid_month", axis_label="Covid(명)"), "right"
)
...
for x in [
    ...
    4000,
    5000,
]:  # adding linear regressed data(df_Credit2) to dataframe(df_Credit1)
    y = [x]
    for i in range(0, 13):
        y.append(
            float(df_Credit2.loc[i, "coef"]) * x + df_Credit2.loc[i, "y_intercept"]
        )
    df_Credit1 = df_Credit1.append(
        pd.Series(y, index=df_Credit1.columns), ignore_index=True
    )
...
def confirmed():
    return p_DateConfirmed


def confirmed2():
    return p_DateConfirmed2
